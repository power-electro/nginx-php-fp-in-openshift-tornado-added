...
nohup sh -c" python ${DIR}/misc/ng_php_conf_hooks.py "   > $OPENSHIFT_LOG_DIR/ng_php_conf_hooks.log 2>&1 &

#---starting nginx ----
#${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/nginx/sbin/nginx -c  ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/nginx/conf/nginx.conf
nohup sh -c "${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/nginx/sbin/nginx -c  ${OPENSHIFT_HOMEDIR}/app-root/repo/srv/nginx/conf/nginx.conf" > $OPENSHIFT_LOG_DIR/nginx_run.log 2>&1 &  tail -f $OPENSHIFT_LOG_DIR/nginx_run.log
nohup sh -c"${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/sbin/php-fpm " > $OPENSHIFT_LOG_DIR/php_run.log 2>&1 & tail -f $OPENSHIFT_LOG_DIR/php_run.log
#cd ${OPENSHIFT_HOMEDIR}/app-root/runtime/repo/.openshift/action_hooks/
#nohup sh -c "./build " > $OPENSHIFT_LOG_DIR/php_run.log 2>&1 & tail -f $OPENSHIFT_LOG_DIR/php_run.log

#---stoping nginx ----
 killall nginx # > $OPENSHIFT_LOG_DIR/nginx_stop.log 2>&1 &
 killall php-fpm #> $OPENSHIFT_LOG_DIR/php-fpm_stop.log 2>&1 &
